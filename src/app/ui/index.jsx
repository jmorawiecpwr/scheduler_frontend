  ...
  return (
    <div className={`relative ${className}`} ref={containerRef}>
      ...
      <div
        ...
        className={`
          w-full px-3 py-2 rounded-lg border bg-white text-slate-900 cursor-pointer min-h-[38px]
          focus:outline-none focus:ring-2 focus:ring-slate-500 focus:border-slate-500
          ${error ? 'border-red-300' : 'border-slate-200'}
        `}
      >
        {selectedLabels.length > 0 ? (
          ...
          <span className="text-slate-400 text-sm">Wybierz opcje...</span>
        )}
      </div>
          ...
          {options.map((option) => (
            ...
            >
              <input
                ...
                readOnly
                className="w-4 h-4 rounded border-slate-300 text-slate-900 focus:ring-slate-500 mr-2"
              />
...
export function ModalHeader({ children, onClose, className = '' }) {
  return (
    <div className={`flex items-center justify-between px-6 py-4 border-b border-slate-200 shrink-0 ${className}`}>
...
export function ModalFooter({ children, className = '' }) {
  return (
    <div className={`flex justify-end gap-3 px-6 py-4 border-t border-slate-200 bg-slate-50 rounded-b-2xl shrink-0 ${className}`}>
      {children}
    </div>
  );
}
...
export function Spinner({ size = 'md', className = '' }) {
  ...
  return (
    <svg
      className={`animate-spin text-slate-600 ${sizes[size]} ${className}`}
      fill="none"
      viewBox="0 0 24 24"
    >
      <circle
